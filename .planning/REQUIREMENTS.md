...
- [ ] **INV-06** — User can confirm that every file under `raw` has a resolved status; assertion holds: n_profiled + n_listed_not_profiled + n_read_failed = n_total_files, zero files of unknown status; `read-failed` covers any readable-extension file whose import failed (locked, corrupt, or XLSX sheet-count shortfall)
