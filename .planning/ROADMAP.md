...
**Goal**: Users can confirm that every patient has a stable, derivation-documented linkage key (pecan_ID) attached to pipeline outputs, with cardinality assertions passing and r7/r8/r9 linkage reach explicitly tested
...
**Success Criteria** (what must be TRUE):
  ...
  4. User can confirm pecan_ID is attached per PCM-D-18 with row counts unchanged (41,150 merged; 13,890 cohort), zero blank pecan_ID where ENCRYPTED_MRN is non-blank, and zero patients gaining a second pecan_ID after attachment
...
**Depends on**: Phase 19, Phase 20 (99_run_all.sas must include programs 19 and 20)
**Requirements**: RUN-01, FIX-01
**Success Criteria** (what must be TRUE):
  1. User can execute `99_run_all.sas` in a clean SAS session and have programs 1-8, 10b, 16b, 17, 18, 19, and 20 run in order, each as a separate batch invocation per PCM-C-05, with exit code 3 on any abort
