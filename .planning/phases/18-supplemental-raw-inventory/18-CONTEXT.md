...
2. **Gap-fill counts (PCM-D-15 input)** — On matched IDs only, for every file with >0 matched IDs (r1, r2, r3, r4, r5, r6, r9), report per column:
   ...
   r7 and r8 are excluded until PCM-D-16 is resolved. Write `qc\18_gap_candidates.txt`. Phase 18 ends with a `%let D15_APPROVED=0` gate; Gerard reviews the output, records the decision in `docs/DECISIONS.md`, then sets the flag before Phase 17 can proceed.
...
- **D-03:** Phase 18 ends with a `%let D15_APPROVED=0` gate — same pattern as Phase 17 Checkpoint 1 (`DOMAIN_MAP_APPROVED`). The program writes `qc\18_gap_candidates.txt`. Gerard reviews, records the approved columns in `docs/DECISIONS.md`, then sets the flag to 1. Phase 17 cannot run until this gate is cleared.
...
- `sas/16_raw_inventory.sas` — Phase 16 program; established the import pattern to reuse: `%import_csv` (PROC IMPORT, `guessingrows=max`) and `%import_xlsx` (libname `xin` with the XLSX engine, sheets enumerated from `dictionary.tables` and copied via `call execute`), key detection (PRECEDE+ID or `STUDYID`), IN_BASE bucketing via `dictionary.columns`, and the `%assert_base` guard.
...
- `sas/00_config.sas` — `g_path`, `qc_path`, `logs_path`; `raw_path` to be added (see canonical refs)
- `sas/17_summary_stats_by_domain.sas` — `%let DOMAIN_MAP_APPROVED=0` gate pattern to replicate for `%let D15_APPROVED=0`

### Established Patterns
- All programs `%include "C:\Master_Renamed_same_format_accross\sas\00_config.sas"` as first step, then `libname g "&g_path";` and `%assert_base` before any reference to `g.analysis_base` — the config does not assign the libname, and Phase 16's first run produced silent zero coverage for exactly this reason
...
- Comparing raw to base values for the conflict count requires type-aware comparison: cast both sides with `strip(cats())` for equality, and note that char/num type differences between raw and base on the same column name are themselves a finding to report.
