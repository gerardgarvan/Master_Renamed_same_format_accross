...
- [x] **PREP-08** — User can verify negative operative intervals (`rt_INCISE_to_DRESS_mins`, `rt_RM_START_to_INCISION_mins`, `rt_RM_START_to_RM_END_mins`) are flagged via `rt_*_neg` columns in `g.master_data_merged` (flag-dont-null design, PCM-D-08 precedent); MRG-07 flag counts: 52 / 15 / 0 (Phase 3 Plan 06, verified 2026-09-14)
- [x] **PREP-09** — User can see every other `rt_*` variable scanned for negatives with counts written to `logs/03_negtime_mdN.txt` per source; nothing is modified; PCM-D-10 closed — only `rt_ANCHOR_to_*_days` negatives found (legitimate offsets) (Phase 3 Plan 06, verified 2026-09-14)
...
- [x] **SUMM-DOMAIN-DISC** — User can run `17_summary_stats_by_domain.sas` and have it discover the year variable, build the extension KEEP= list, resolve key type at macro time, and produce a sentinel-applicability list — all written to `qc/17_discovery.txt` before any statistics execute (Wave 0 discovery, Phase 17)
- [x] **SUMM-DOMAIN-MAP** — User can review a domain assignment table (`g.var_domain_map`) with one row per analysis variable, each carrying a rationale and a `stat_route` (continuous/categorical/binary), produced after a three-tier dictionary match and Checkpoint 1 human approval (Wave 1 domain map, Phase 17)
- [x] **SUMM-DOMAIN-STATS** — User can verify pooled and per-year descriptive statistics (PROC MEANS for continuous, PROC FREQ for categorical/binary) are computed for all five clinical domains with sentinel recoding scoped to the applicability list and small-cell suppression (≤11) applied before any output step (Wave 2 statistics, Phase 17)
- [x] **SUMM-DOMAIN-BOOK** — User can open `qc/17_summary_stats_by_domain.xlsx` and find a single workbook with KEY sheet leftmost, domain sheets D1–D5 (D3 absent pending DATALINES fix — acknowledged Checkpoint 2, 2026-09-10), Crosswalk, and QC; UF blue (#0021A5) headers; suppressed cells labelled `--`; pooled and per-year columns under spanning headers (Wave 3 workbook, Phase 17)
