...
No variable appears in two domains. Every variable carries a one-line `domain_rationale`
recorded in the crosswalk (§5), including the unambiguous ones, so the map can be reviewed
variable by variable rather than accepted as a whole.

### 2.1 Assignment rule

Assignment is a judgment call, so it is made against a stated rule rather than case by case.

1. **Timing first.** Where in the episode of care was the value captured? Before the case, in
   the OR record, or after. This resolves most ambiguity: BMI from the preop clinic visit is
   D2, a weight in the anesthesia record is D4; a comorbidity coded from history is D2, the
   same condition appearing postoperatively is D5; case duration is D4 because it describes
   the case, length of stay is D5 because it describes what followed.
2. **Analytic role second.** Where timing does not settle it, a value known before incision
   goes to D2 and a value realized afterward goes to D5.
3. **Instrument membership overrides both.** A variable that is part of a named cognitive
   instrument goes to D3 regardless of when it was administered.

`domain_rationale` names which of these three applied, in the reviewer's own words — for
example, "preop clinic capture, timing rule" or "dCDT subscore, instrument rule".
...
| PCM-xx | A crosswalk sheet lists every variable, its domain, its `domain_rationale`, its type, its source dataset and its data-dictionary label |
| PCM-xx | Every variable assigned to a domain has a non-empty `domain_rationale` citing which assignment rule (§2.1) applied; a blank rationale fails the phase |
...
3. Assign each matched variable to D1–D5 using the rule in §2.1. Record a one-line `domain_rationale` for each. Store both as `g.var_domain_map` and export to the crosswalk sheet for review.
4. Flag for closer review: any variable whose rationale cites rule 2 (analytic role), since those are the assignments timing could not settle.
5. **Checkpoint:** Gerard reviews the domain assignment before any statistics are produced. The review is variable-by-variable against the rationale column, not a sign-off on the map as a whole.

**Wave 2 — Statistics**
6. Apply sentinel recoding (`-999`, `NULL`, empty) to a working copy. Log how many values were recoded per variable.
7. Continuous variables: `PROC MEANS` with the statistic set in PCM-xx, `BY` domain.
8. Categorical variables: `PROC FREQ` with `/ MISSING` to keep missing visible, then split missing into its own column.
9. If D-03 resolves to per-year, repeat 7–8 with the year classifier and stack.

**Wave 3 — Assembly**
10. Build the workbook with `ODS EXCEL`, one sheet per domain, KEY leftmost, crosswalk and QC sheets last.
11. QC sheet: source dataset name, row count, run datetime, sentinel recode counts, count of suppressed cells, count of variables per domain, count of variables assigned by each of the three rules.
12. **Checkpoint:** review of the issued workbook against the data dictionary.
...
- Every assigned variable has a non-empty `domain_rationale`, and the rule cited is one of the three in §2.1.
